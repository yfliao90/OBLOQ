...
接下来的项目，需要用一个按键远程控制台灯的亮灭。一个OBLOQ模块将按键的状态发送到物联网，物联网接着将接收到的按键状态发送到另一个OBLOQ模块，测试接收的模块通过判断按键状态来控制台灯亮灭

因为物联网端需要知道按键的状态，所有这里需要在OBLOQ数据库里建立一个按键设备，

1.创建**设备：Button**。

注册账号并且登录，会进入个人的物联网设备界面，在设备列表的左上角有个**添加设备**的按钮，在**设备名称**中输入：Button，再点击后面的**添加设备**按钮即可。

![创建设备](C:\Users\jason\Desktop\GitHub\OBLOQ\OBLOQ教程\img\创建设备.png)



2.查看设备是否创建成功

在完成步骤1之后，查看设备列表里面是不是出现一个Button设备，如果有，表示创建成功。

![创建设备成功](C:\Users\jason\Desktop\GitHub\OBLOQ\OBLOQ教程\img\创建设备成功.png)
...
如果使用的是台灯，因为正常台灯的输入电压是220V，需要直接接到家庭用电的插座上才能正常工作，而控制板只能输出5V电压，不能使台灯正常工作。如果希望UNO主控板能够控制台灯，我们需要通过继电器模块来完成弱点对强电的控制。下面完成这个改动。

1.将继电器连接到台灯的输入端口

台灯的插座有两根线，一根火线一根地线，将其中一根线剪断串连接上继电器，一端接入NO（常开），一段接入COM（公共端）即可。如果不想破坏台灯的电线，也可以找根平时不用的带插头的线来改装，如下图：

图

2.将烧录好接收端程序的UNO连接到继电器

图

3接通台灯电源，插入台灯插座即可。接通UNO电源，可以USB供电，也可以电池供电。
...
如果只有一套OBLOQ模块和主控，按键功能依旧能够通过在OBLOQ-web网站虚拟发送按键开关的信息实现。
...
1.登录物联网，在设备列表中找到按键设备：

![创建设备成功](C:\Users\jason\Desktop\GitHub\OBLOQ\OBLOQ教程\img\创建设备成功.png)



2.点击**Button**设备后面的**设备消息**按钮，出现如下消息界面：

![设备消息](C:\Users\jason\Desktop\GitHub\OBLOQ\OBLOQ教程\img\设备消息.png)



3，发送消息，在消息界面的左上角输入消息内容再点击后面的**发送消息**按钮即可。发送消息0关闭台灯，发送消息1打开台灯。

![发送消息1](C:\Users\jason\Desktop\GitHub\OBLOQ\OBLOQ教程\img\发送消息1.png)

4,查看消息是否发送成功，在点击发送消息按钮后，如果消息列表里面出现刚才输入的消息，表示发送成功。

![消息发送成功](C:\Users\jason\Desktop\GitHub\OBLOQ\OBLOQ教程\img\消息发送成功.png)
